...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  // Koneksi ke database sunset
  $conn = new mysqli("localhost", "root", "", "projectelit");  // Pastikan ke database sunset
  ...
  // Loop untuk menyimpan data fasilitas
  foreach ($facilityNames as $i => $facility_name) {
    ...
    // Ganti nama tabel facilities menjadi sunset_facilities
    $sql = "INSERT INTO sunset_facilities (facility_name, kondisi, status, description, photo, group_name, tanggal) 
    ...
    <div class="main-content flex-1 flex flex-col transition-all duration-300" id="mainContent">
      ...
      <main class="p-8 bg-gray-50 h-full overflow-y-auto mt-16">
        <div id="facilities-table">
          <h1 class="text-3xl font-bold mb-6 text-black">Laporan Sunset</h1>
          <form action="" method="POST" enctype="multipart/form-data" id="facilityForm">
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden mb-8 p-4">
              <h2 class="text-xl font-semibold text-blue-600 mb-4">Tanggal Input</h2>
